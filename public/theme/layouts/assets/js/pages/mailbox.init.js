...
function loadMailData(e) {
    document.querySelector("#mail-list").innerHTML = "", Array.from(e).forEach(function(e, t) {
        var a = e.readed ? "" : "unread",
            l = e.starred ? "active" : "",
            r = e.counted ? "(" + e.counted + ")" : "";
        document.querySelector("#mail-list").innerHTML += '<li class="' + a + '">        <div class="col-mail col-mail-1">            <div class="form-check checkbox-wrapper-mail fs-14">                <input class="form-check-input" type="checkbox" value="' + e.id + '" id="checkbox-' + e.id + '">                <label class="form-check-label" for="checkbox-' + e.id + '"></label>            </div>            <input type="hidden" value=' + e.userImg + ' class="mail-userimg" />            <button type="button" class="btn avatar-xs p-0 favorite-btn fs-15 ' + l + '">            <i class="ri-star-fill"></i>            </button>            <a href="javascript: void(0);" class="title"><span class="title-name">' + e.name + "</span> " + r + '</a>        </div>        <div class="col-mail col-mail-2">            <a href="javascript: void(0);" class="subject"><span class="subject-title">' + e.title + '</span> – <span class="teaser">' + e.description + '</span>            </a>            <div class="date">' + e.date + "</div>        </div>    </li>", favoriteBtn(), emailDetailShow(), emailDetailChange(), checkBoxAll()
    })
}
...
function checkBoxAll() {
    ...
    var e = document.querySelectorAll(".mail-primary .checkbox-wrapper-mail input");

    function a() {
        var e = document.querySelectorAll(".mail-primary .checkbox-wrapper-mail input"),
            t = document.querySelectorAll(".mail-primary .checkbox-wrapper-mail input:checked").length;
    ...
    Array.from(e).forEach(function(e) {
        e.addEventListener("click", function(e) {
            var t = document.querySelectorAll(".mail-primary .checkbox-wrapper-mail input"),
                a = document.getElementById("checkall"),
                l = document.querySelectorAll(".mail-primary .checkbox-wrapper-mail input:checked").length;
...
getJSON("mail-list.init.json", function(e, t) {
    null !== e ? console.log("Something went wrong: " + e) : loadMailData(allmaillist = t[0].primary)
}), Array.from(document.querySelectorAll(".mail-list a")).forEach(function(a) {
    a.addEventListener("click", function() {
        var t, e = document.querySelector(".mail-list a.active");
        e && e.classList.remove("active"), a.classList.add("active"), loadMailData(a.querySelector(".mail-list-link").hasAttribute("data-type") ? (t = a.querySelector(".mail-list-link").innerHTML, allmaillist.filter(e => e.labeltype === t)) : (t = a.querySelector(".mail-list-link").innerHTML, document.getElementById("mail-list").innerHTML = "", "All" != t ? allmaillist.filter(e => e.tabtype === t) : allmaillist))
    })
}), ClassicEditor.create(document.querySelector("#email-editor")).then(function(e) {
    e.ui.view.editable.element.style.height = "200px"
}).catch(function(e) {
    console.error(e)
}), scrollToBottom(), removeItems(), removeSingleItem();
...
function emailDetailChange() {
    Array.from(document.querySelectorAll(".message-list li")).forEach(function(r) {
        r.addEventListener("click", function() {
            var e = r.querySelector(".checkbox-wrapper-mail .form-check-input").value,
                e = (document.querySelector(".remove-mail").setAttribute("data-remove-id", e), r.querySelector(".subject-title").innerHTML),
                a = (document.querySelector(".email-subject-title").innerHTML = e, r.querySelector(".title-name").innerHTML),
...
document.querySelectorAll(".email-chat-list a").forEach(function(l) {
    ...
    l.classList.contains("active") && (document.getElementById("emailchat-detailElem").style.display = "block", e = document.querySelector(".email-chat-list a.active").querySelector(".chatlist-user-name").innerHTML, t = document.querySelector(".email-chat-list a.active").querySelector(".chatlist-user-image img").getAttribute("src"), document.querySelector(".email-chat-detail .profile-username").innerHTML = e, document.getElementById("users-conversation").querySelectorAll(".left .chat-avatar").forEach(function(e) {
        ...
        var t = document.querySelector(".email-chat-list a.active"),
            t = (t && t.classList.remove("active"), this.classList.add("active"), scrollToBottom(), l.querySelector(".chatlist-user-name").innerHTML),
...
}), resizeEvent(), window.onresize = resizeEvent, Array.from(document.querySelectorAll(".toggle-email-panel")).forEach(function(e) {
    e.addEventListener("click", function() {
        document.querySelector(".email-wrapper .email-panel").classList.contains("show") ? document.querySelector(".email-wrapper .email-panel").classList.remove("show") : document.querySelector(".email-wrapper .email-panel").classList.add("show")
    })
}), Array.from(document.querySelectorAll(".mail-user-dropdown")).forEach(function(t) {
    t.querySelectorAll(".dropdown-menu .dropdown-item").forEach(function(e) {
        e.addEventListener("click", function() {
            t.querySelector(".mail-profile-img").src = e.querySelector(".mail-user-img").src
        })
    })
});
var searchList = document.getElementById("searchResultList");
searchList.addEventListener("keyup", function() {
    var t, e = searchList.value.toLowerCase();
    t = e, loadMailData(allmaillist.filter(function(e) {
        return -1 !== e.name.toLowerCase().indexOf(t.toLowerCase()) || -1 !== e.description.toLowerCase().indexOf(t.toLowerCase()) || -1 !== e.title.toLowerCase().indexOf(t.toLowerCase())
    }))
});